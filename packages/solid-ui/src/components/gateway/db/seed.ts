...
function generateRandomLog(routeId?: string) {
    ...
    const latency =
        Math.random() < 0.9 ? Math.floor(Math.random() * 2000) + 50 : Math.floor(Math.random() * 5000) + 3000;
...
export async function seedLogs(count: number = 100) {
    ...
    for (let i = 0; i < count; i += batchSize) {
        ...
        for (let j = 0; j < batchCount; j++) {
            // 随机选择一个路由 ID（如果存在路由）
            const routeId = routes.length > 0 ? routes[Math.floor(Math.random() * routes.length)].id : undefined;
...
export async function seedSettings() {
    ...
    const randomSettings = {
        ...
        logLevel: ['debug', 'info', 'warn', 'error'][Math.floor(Math.random() * 4)] as
            | 'debug'
            | 'info'
            | 'warn'
            | 'error',
