...
export const Dashboard = () => {
  const stats = [
    { label: '总请求数', value: '1,284,392', trend: '+12.5%', isUp: true, icon: Zap, color: 'text-blue-600', bg: 'bg-blue-100' },
    { label: '平均延迟', value: '124ms', trend: '-2.1%', isUp: false, icon: Clock, color: 'text-purple-600', bg: 'bg-purple-100' },
    { label: '错误率', value: '0.04%', trend: '+0.01%', isUp: true, icon: AlertTriangle, color: 'text-amber-600', bg: 'bg-amber-100' },
    { label: '活跃节点', value: '12', trend: '稳定', isUp: true, icon: Server, color: 'text-emerald-600', bg: 'bg-emerald-100' },
  ];
        ...
        {stats.map((stat) => (
          <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class={`flex items-center text-xs font-medium ${stat.isUp ? 'text-green-600' : 'text-red-600'}`}>
                {stat.trend}
                {stat.isUp ? <ArrowUpRight class="w-3 h-3 ml-0.5" /> : <ArrowDownRight class="w-3 h-3 ml-0.5" />}
              </div>
            </div>
